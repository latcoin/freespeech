...
// see: https://groups.google.com/forum/#!topic/freespeech-dev/uKjFGSw24yA
...
namespace DhtProxy
{
    ...
    bool dhtgetReplyReceived(const CDHTGetReply& reply, CNode* pfrom)
    {
        ...
        if( !reply.vchBencodedData.size() ) {
            ...
            // No reply - these fields are not used, we just want cast in freespeech.cpp:dhtget to fail
            dht_reply_data_done_alert dd("","",false,false,false);
            dhtgetMapPost(ih, dd);
        } else {
            lazy_entry v;
